...
def export_deploy_cfg(env: ManagerBasedRLEnv, log_dir, history_length: int = 0, use_encoder: bool = True):
    ...
    Args:
        ...
        log_dir: Directory to save the configuration file.
        history_length: Number of historical time steps for encoder input.
        use_encoder: Whether to use dual network architecture (encoder + policy).
    """
    ...
    for obs_name, obs_cfg in obs_terms:
        ...
        # clean cfg
        term_cfg = term_cfg.to_dict()
        # Remove history_length from individual observation terms (use global history_length instead)
        for _ in ["func", "modifiers", "noise", "flatten_history_dim", "history_length"]:
            if _ in term_cfg:
                del term_cfg[_]
